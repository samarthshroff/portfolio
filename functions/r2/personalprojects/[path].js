export async function onRequest({ params, env }) {
  try {
    console.log("Function invoked for route: /r2/personalprojects/" + params.path);
    console.log(`Raw params.path: "${params.path}"`);
    
    const decodedPath = decodeURIComponent(params.path);
    console.log(`Decoded path: "${decodedPath}"`);
    
    // Construct the R2 key specifically for personalprojects
    const key = `assets/personalprojects/${decodedPath}`;
